...
try {
  ...
  async function storeMatchResult(userIdA, userIdB, score) {
    try {
      ...
      batch.set(userAMatchRef, {score, updatedAt: admin.firestore.FieldValue.serverTimestamp()}, { merge: true });

      // Store score under second user
      const userBMatchRef = db.collection("users").doc(userIdB).collection("matches").doc(userIdA);
      batch.set(userBMatchRef, {score, updatedAt: admin.firestore.FieldValue.serverTimestamp()}, { merge: true });

      await batch.commit();
      console.log(`Successfully stored match between ${userIdA} and ${userIdB} with score ${score}`);
      return true;
    } catch (error) {
      console.error(`Error storing match between ${userIdA} and ${userIdB}:`, error);
      return false;
    }
  }
   ...
   * @return {Promise} - Promise resolving when all matches are computed
   */
  ...
  firestoreFunctions = {
    ...
    onUserUpdate: functions.firestore
      ...
      .onUpdate(async (change, context) => {
        ...
        // Check if questionnaire data exists and was changed
        if (newData.questionnaire && 
            (!oldData.questionnaire || 
             JSON.stringify(newData.questionnaire) !== JSON.stringify(oldData.questionnaire))) {
          console.log(`User ${userId} updated their questionnaire, computing matches`);
          return computeMatchesForUser(userId);
        }
        
        console.log(`No relevant changes for user ${userId}, skipping match computation`);
        return null;
      }),
